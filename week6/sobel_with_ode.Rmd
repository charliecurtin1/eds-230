...
## we run everything after this for a single parameter set, not all of them
```{r sen}
source("../week6/dpopgrowth.R")
...
## a negative growth rate doesn't make sense, so we remove negative values from our parameter set
...
## run our function for each year for 200 years
...
# run the ode and for each timestep we get a population
...
## plot results for the initial results
ggplot(result, aes(time, P)) + 
  geom_point()

### ways to summarize our output
...
thresh = 150
## which row is greater than this threshold
...
ggplot(result, aes(time, P)) + 
  ...
  labs(y="Population", 
       title="When do we get to 150")

## what time does it get to 50% of carrying capacity
...
## returns which year do we reach a specified threshold of population
...
## outputs a boxplot of the distribution of population for each parameter set, as well as the the distribution of the years in which the population passes the specified threshold, which is 90% of the carrying capacity
```
